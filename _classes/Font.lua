...
function Font:_newBMTable(bmt, size)
   ...
   for i = 0, com.pages - 1 do
      pages[i] =
         change_img(ImageData:_decode(love.data.decode("string", "base64", assert(bmt[i], "bad font table: no page " .. i))))
   end
   local glyphs = {}
   n._glyphs = glyphs
   n._pages = pages
   if bmt.fnt.char then
      for _, v in next, bmt.fnt.char do
         ...
         local imgd = assert(pages[v.page], "No page " .. v.page .. " for char " .. v.id):_window(
            v.x,
            v.y,
            v.width,
            v.height
         )
         glyphs[v.id] = GlyphData:_new(
            v.id,
            v.xoffset,
            v.yoffset,
            v.xadvance,
...
function Font:_arrange(txt, wrap)
   local locs = {}
   local glyphs = rawget(self, "_glyphs")
   local kerns = rawget(self,"_kerns")
   -- V this joke is now deprecated because it's yp not yo
   -- yo waddup
   local xp, yp = 0, rawget(self,"_baseline")
   local prevcode
   local function doit(codepoint)
      if wrap and xp >= wrap then
         xp = 0
         yp = yp + rawget(self, "_lineheight")
      end
      local glyph = glyphs[codepoint]
      if glyph~=nil then
         locs[#locs + 1] = {
            glyph = glyph,
            x = xp + rawget(glyph,"_xoffset") + (prevcode and kerns[prevcode] and kerns[prevcode][codepoint] or 0),
            y = yp + rawget(glyph, "_yoffset"),
         }
         xp = xp + glyph:getAdvance()
         prevcode=codepoint
      end
